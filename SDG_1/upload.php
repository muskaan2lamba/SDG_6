		...
		if(!empty($_FILES['myfile']['name']) && isset($_FILES['myfile'])){
			...
			$size1 = $_FILES['myfile']['size'];
					...
					if (move_uploaded_file($file1, $destination1) )
					{
						...
						$sql = "INSERT INTO `internship_data`(`roll_no`, `topic`,`year_new`,`gpa`, `starting_date`, `end_date`,`company_name`, `approval_status`, `completion_status`, `certificate`,`feedback`,`duration`,`letter`,`myfile`,`comment`) VALUES ('$roll_no','$topic','$year','$gpa','$starting_date','$end_date','$company_name','Pending','None','Not Uploaded Yet','Give Feedback','$duration','$letter','".$destination1."','No comment')";
						if ($conn->query($sql))
						{
